...
export default function BroadcastModal({ open, onOpenChange }: BroadcastModalProps) {
  ...
  // Fetch user count and phone numbers based on category
    ...
    try {
      ...
      if (category === "all") {
        ...
        const phoneNumbers = usersSnapshot.docs
          ...
          .filter((phoneNumber): phoneNumber is string => phoneNumber !== undefined && phoneNumber !== "");
        setUserCount(phoneNumbers.length);
        return phoneNumbers;
        ...
        const phoneNumbers = snapshots
          .flatMap((snap) =>
            snap.docs.map((doc) => doc.data().phoneNumber).filter((phoneNumber): phoneNumber is string => phoneNumber !== undefined && phoneNumber !== "")
          )
          .filter((phoneNumber, index, self) => self.indexOf(phoneNumber) === index); // Remove duplicates
        setUserCount(phoneNumbers.length);
        return phoneNumbers;
      } else {
        setUserCount(0);
  ...
  const onSubmit = async (data: BroadcastData) => {
    ...
    try {
      const recipients = await fetchUserData();
      if (recipients.length === 0) {
        toast.error("No recipients found for the selected category.");
        return;
      }
      console.log("Sending to Firestore:", { message: data.message, recipients, createdAt: serverTimestamp() });
