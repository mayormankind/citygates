...
export default function Stores() {
  ...
  const handleStoreStatusChange = async (storeId: string, isActive: boolean) => {
    setLoading(true);
              ...
              {loading ? (
                ...
                filteredStores.map((store) => (
                  <TableRow key={store.id}>
                    <TableCell>
                      <Image src={store.image} alt={store.name} width={1000} height={1000} className="w-12 h-12 rounded-md"/>
                    </TableCell>
                    <TableCell>{store.name}</TableCell>
                    <TableCell>
                      <Switch
                        checked={store.status === "active"}
                        onCheckedChange={(checked:boolean) => handleStoreStatusChange(store.id, checked)}
                        disabled={loading}
                      />
                    </TableCell>
